...
# ---------------- Data files & defaults ----------------
# Note: DATA_DIR will be set at runtime inside the App to support Android writable user_data_dir.
...
def ensure_default_files(data_dir):
    os.makedirs(data_dir, exist_ok=True)
    sc_file = os.path.join(data_dir, SCENARIO_FILE)
    ud_file = os.path.join(data_dir, USER_DATA_FILE)
    rp_file = os.path.join(data_dir, REPORTS_FILE)

    if not os.path.exists(sc_file):
        save_json(sc_file, DEFAULT_SCENARIOS)
...
class ScenarioListScreen(Screen):
    ...
    def populate_list(self):
        ...
        data_dir = MDApp.get_running_app().data_dir
        scenarios = load_json_default(os.path.join(data_dir, SCENARIO_FILE), [])
...
class ScenarioScreen(Screen):
    ...
    def on_choose(self, idx):
        ...
        # save answer
        data_dir = MDApp.get_running_app().data_dir
        ud_path = os.path.join(data_dir, USER_DATA_FILE)
        data = load_json_default(ud_path, [])
        data.append({
    ...
    def send_report(self):
        data_dir = MDApp.get_running_app().data_dir
        rp_path = os.path.join(data_dir, REPORTS_FILE)
        reports = load_json_default(rp_path, [])
        reports.append({
            "timestamp": datetime.utcnow().isoformat() + "Z",
            "scenario_id": self.scenario.get("id"),
            "note": f"Người dùng gửi báo cáo cho '{self.scenario.get('title')}'."
        })
        save_json(rp_path, reports)
...
class HistoryScreen(Screen):
    ...
    def populate(self):
        ...
        data_dir = MDApp.get_running_app().data_dir
        data = load_json_default(os.path.join(data_dir, USER_DATA_FILE), [])
        reports = load_json_default(os.path.join(data_dir, REPORTS_FILE), [])
        from kivymd.uix.list import OneLineListItem
    ...
    def clear_history(self):
        data_dir = MDApp.get_running_app().data_dir
        save_json(os.path.join(data_dir, USER_DATA_FILE), [])
        save_json(os.path.join(data_dir, REPORTS_FILE), [])
        self.populate()
    ...
    def send_sample_report(self):
        ...
        save_json(os.path.join(data_dir, REPORTS_FILE), reports)
        self.populate()
...
# ---------------- App ----------------
class CanhGiacApp(MDApp):
    data_dir = None  # will be set in build()

    def build(self):
        # determine writable data directory (works on Android and desktop)
        if platform == "android":
            # on Android use user_data_dir
            self.data_dir = self.user_data_dir
        else:
            # desktop: place data_files next to script (so it's easy to inspect)
            base = os.path.dirname(__file__) if '__file__' in globals() else os.getcwd()
            self.data_dir = os.path.join(base, "data_files")

        ensure_default_files(self.data_dir)

        # Material 3 (if available)
        try:
